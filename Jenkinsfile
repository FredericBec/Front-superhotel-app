pipeline {
    ...
    stages {
        ...
        stage('Code analysis') {
            steps {
                script{
                    bat '''sonar-scanner.bat -D"sonar.projectKey=Front-superhotel" -D"sonar.sources=." -D"sonar.host.url=http://localhost:9000" -D"sonar.login=sqp_bdecf197fa6f895306ead10cdb80dca898cc874c"'''
                }
            }
        }
